    ...
    let editandoId = null; // <-- controla edição
    ...
    function renderTabela() {
        ...
        receitas.forEach(receita => {
            const [ano, mesNum] = receita.mes.split('-');
            ...
            tableHTML += `
                <tr>
                    ...
                    <td>
                        <button class="action-btn" onclick="editarReceita(${receita.id})">Editar</button>
                        <button class="action-btn delete-btn" onclick="deleteReceita(${receita.id})">Excluir</button>
                    </td>
                </tr>`;
        });
    ...
    function montarGrafico() {
        ...
        if (currentChartType === 'pie') {
            ...
            datasets = [
                { label: 'Salário Bruto', data: dataSalarioBruto, backgroundColor: 'rgba(0, 158, 227, 0.7)', borderColor: '#009ee3', borderWidth: 2 },
                { label: 'Descontos', data: dataDescontos, backgroundColor: 'rgba(255, 59, 48, 0.7)', borderColor: '#ff3b30', borderWidth: 2 },
                { label: 'Salário Líquido', data: dataSalarioLiquido, backgroundColor: 'rgba(52, 199, 89, 0.7)', borderColor: '#34c759', borderWidth: 2 }
            ];
        }
        chartInstance = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: { callbacks: { label: (context) => `R$ ${context.parsed.y?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` } }
                },
                scales: currentChartType !== 'pie' ? {
                    y: { beginAtZero: true, ticks: { callback: (value) => `R$ ${value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` } }
                } : {}
            }
        });
    }
    ...
    async function handleSubmit(e) {
        ...
        try {
            if (editandoId) {
                const { data, error } = await supabase
                    .from('receitas')
                    .update({ mes: mesInput, salario, descontos, descricao })
                    .eq('id', editandoId)
                    .select();
                if (error) throw error;
                const index = receitas.findIndex(r => r.id === editandoId);
                if (index !== -1) receitas[index] = data[0];
                editandoId = null;
                showNotification('Receita atualizada com sucesso!');
            } else {
                const { data, error } = await supabase
                    .from('receitas')
                    .insert([{ mes: mesInput, salario, descontos, descricao }])
                    .select();
                if (error) throw error;
    ...
    function editarReceita(id) {
        ...
        if (!receita) return;
        editandoId = id;
        document.getElementById('mes').value = receita.mes;
        document.getElementById('salario').value = receita.salario.toFixed(2);
        document.getElementById('descontos').value = receita.descontos.toFixed(2);
